...
> #### 📊 Фильтрация в `VirtualTable`
>
> Компонент принимает массив фильтров через проп `:filters`. Поддерживаемые операторы:
>
...
> **Глобальный поиск:**
> ```js
> { key: '*', value: 'поисковый запрос' } // простой вариант
> { key: 'global', operator: 'contains', value: 'поисковый запрос' } // с оператором/функцией
> ```
>
> **Логика комбинирования:**
> * Между разными колонками (`status` и `growth`) — `AND`.
> * Внутри одной колонки при нескольких значениях (статус «А» **или** «Б», `growth` `10.8` **или** `0`) — по умолчанию `OR`.
> * Исключение: внутри одной колонки для операторов сравнения (`>`, `>=`, `<`, `<=`, `!=`) — `AND` (например, `growth >= 10` и `growth <= 50`).
>
